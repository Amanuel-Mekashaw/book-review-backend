...
public class AuthorDetailController {
  ...
  public ResponseEntity<ApiResponse<AuthorDetail>> createAuthorDetail(@RequestBody AuthorDetailDTO authorDetailDTO) {
    User user = userRepository.findById(authorDetailDTO.getUserId())
        .orElseThrow(() -> new EntityNotFoundException("user by this id" + authorDetailDTO.getUserId() + " not found"));
    ...
    ApiResponse<AuthorDetail> apiResponse = new ApiResponse<>("Author detail created successfully", 201, authorDetail);
  ...
  public ResponseEntity<ApiResponse<AuthorDetail>> updateAuthorDetail(@RequestBody AuthorDetailDTO authorDetailDTO,
    ...
    if (foundAuthorDetail.getId() == id) {
      ...
      ApiResponse<AuthorDetail> apiResponseError = new ApiResponse<AuthorDetail>("No author detail to be found", 404,
          null);
      return new ResponseEntity<>(apiResponseError, HttpStatus.OK);
    }

    // return new ResponseEntity<ApiResponse<AuthorDetail>>(apiResponse,
    // HttpStatus.CREATED);
  }
